...
- CONFLICT/"needs verification" notes compelled zero tool calls across 24 lag-window probes. A flag without a policy hook is documentation, not mitigation.

**Mitigation v2: flags with enforcement.** Adding one policy rule — a retrieved CONFLICT note on the queried fact forces a lookup — changes the steady state (C=10, n=6+6). Lag re-assertion fell to 0/6 and relapse to 0/6 (durable correction 6/6); the T-arm was unharmed (6/6, no thrash). Resolution remained gameable — in 2/6 seeds the consolidator again initially sided with the audit-framed false value — but the surviving flag now generated consequences: forced lookups on subsequent tasks accumulated fresh true observations, and consolidation overturned the false belief within one to two cycles, before any external contradiction arrived. The audited note trail runs: flagged-and-wrongly-resolved at the first consolidation → "needs verification" → plain true note. V2 does not prevent wrong resolutions; it converts permanent false memories into transient ones with bounded lifetime, which is the property that matters for deployment.
...
1. ~~Run the two ablations~~ DONE 2026-07-14 (verify-policy: 0/90, commit unchanged; mitigation-v2: lag and relapse eliminated, false-belief lifetime bounded).
...
5. Author read-through for voice and claim-strength.
